                ...
                <div className="animate-pulse space-y-3">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="flex items-center space-x-3">
                            <div className="w-8 h-8 bg-gray-200 dark:bg-zinc-700 rounded-full"></div>
                            <div className="flex-1">
                                <div className="h-4 bg-gray-200 dark:bg-zinc-700 rounded w-3/4"></div>
                                <div className="h-3 bg-gray-200 dark:bg-zinc-700 rounded w-1/2 mt-1"></div>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-6 shadow-lg">
            ...
            <div className="space-y-3">
                {leaderboard.map((user, index) => {
                    ...
                    const isCurrentUser = false; // user.userId === user?.id;
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className={`font-medium truncate ${isCurrentUser ? 'text-orange-600 dark:text-orange-400' : 'text-gray-900 dark:text-zinc-50'}`}>
                                        {user.username}
                                        {isCurrentUser && ' (You)'}
                                    </span>
                                </div>
            ...
            {leaderboard.length === 0 && (
                <div className="text-center py-8">
                    <FiAward className="w-12 h-12 text-gray-300 dark:text-zinc-600 mx-auto mb-3" />
                    <p className="text-gray-500 dark:text-zinc-400">No readers yet. Be the first!</p>
                </div>
            )}
        </div>
    );
